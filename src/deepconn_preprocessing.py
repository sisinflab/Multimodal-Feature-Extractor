    ...
    for id_model, m in enumerate(args.model_name):
        ...
        for idx, line in data.iterrows():
            if line['USER_ID'] in token_users:
                ...
                token_users[line['USER_ID']] = '<PAD/>'
                for s in data[data['USER_ID'] == line['USER_ID']]['REVIEW']:
                    token_users[line['USER_ID']] = token_users[line['USER_ID']] + ' ' + s.strip()
                token_users[line['USER_ID']] = clean_str(token_users[line['USER_ID']])
                token_users[line['USER_ID']] = token_users[line['USER_ID']].split(" ")

            if line['ITEM_ID'] in token_items:
                pass
            else:
                token_items[line['ITEM_ID']] = '<PAD/>'
                for s in data[data['ITEM_ID'] == line['ITEM_ID']]['REVIEW']:
                    token_items[line['ITEM_ID']] = token_items[line['ITEM_ID']] + ' ' + s.strip()
                token_items[line['ITEM_ID']] = clean_str(token_items[line['ITEM_ID']])
                token_items[line['ITEM_ID']] = token_items[line['ITEM_ID']].split(" ")

        u = np.array([len(x) for x in token_users.values()])
        x = np.sort(u)
        u_len = x[int(0.85 * len(u)) - 1]

        i = np.array([len(x) for x in token_items.values()])
        y = np.sort(i)
        i_len = y[int(0.85 * len(i)) - 1]

        token_users = pad_sentences(token_users, u_len)
        token_items = pad_sentences(token_items, i_len)

        user_voc = [x for x in token_users.values()]
        item_voc = [x for x in token_items.values()]
        ...
        save_np(npy=text_words_features_vocabulary_users,
                filename='../data/{0}/users_{1}_deepcoon.npy'.format(args.dataset, m.lower()))
        save_np(npy=text_words_features_vocabulary_items,
                filename='../data/{0}/items_{1}_deepconn.npy'.format(args.dataset, m.lower()))
